...
-- local code = token('code',   l.delimited_range('`', true))
...
-- Embedded R.
local rstats = l.load('rstats')
local startR_rule = token('rstats', l.starts_line(P('```')) * l.nonnewline^0    )
local endR_rule   = token('rstats', l.starts_line(P('```')) * l.nonnewline^0)

l.embed_lexer(M, rstats, startR_rule, endR_rule)
